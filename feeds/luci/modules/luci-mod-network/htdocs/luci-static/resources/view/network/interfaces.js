...
return view.extend({
	title: _('Interface Configuration'),

	load: function() {
		return L.resolveDefault(fs.list('/www' + L.resource('view/network/include')), []).then(function(entries) {
			return Promise.all(entries.filter(function(e) {
				return (e.type == 'file' && e.name.match(/\.js$/));
			}).map(function(e) {
				return 'view.network.include.' + e.name.replace(/\.js$/, '');
			}).sort().map(function(n) {
				return L.require(n);
			}));
		});
	...
	render: function(includes) {
		...
		for (var i = 0; i < includes.length; i++) {
			...
			if (includes[i].title != null)
				...
				title = String(includes[i]).replace(/^\[ViewNetworkInclude\d+_(.+)Class\]$/,
